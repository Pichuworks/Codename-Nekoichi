...
def translate(seq_string_list, maxlen = maxlen, maxout = maxlen, sos = 'Ⓑ', unk = '_UNK_', pad = '_PAD_', padfrom = 'start'):
    ...
    encoder_input = np.array([[seq2idx.get(tok, seq2idx[unk]) for tok in var for var in seq_tokens]])
    
    # starting outputs
    decoder_input = np.zeros(shape=(len(seq_string_list), maxlen))
    ...
    for i, str_seq in enumerate(str_toks):
        print('input :', seq_string_list[i])
        print('output:', ''.join(str_seq[i]))
