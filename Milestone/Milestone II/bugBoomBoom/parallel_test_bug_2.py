...
with open('structure_new_150_np.txt') as f:
    for line in f.readlines():
        line = line.strip() 
...
def translate(seq_string_list, maxlen = maxlen, maxout = maxlen, sos = 'Ⓑ', unk = '_UNK_', pad = '_PAD_', padfrom = 'start'):
    ...
    for i, var in enumerate(str_toks):
        # change
        print('Structure RAW: ', ''.join(var))
        print('RNA length ' + str(str_length[i]))
        print('Structure RAW length: ' + str(len(var)))
        print('RAW var id = ' + str(id(var)))
        if len(var) > str_length[i]:
            var = var[:str_length[i]]
        elif len(var) < str_length[i]:
            var = var + ['.' for i in range(str_length[i] - len(var))]
        print('processed var id = ' + str(id(var)))
        print('Structure processed length: ' + str(len(var)))
        # changed
...
for i, test_str_list in enumerate(test_str_seq):
    ...
    print('(%4.1f seconds)\n\n' % (time() - t1))
    with open('fuck_res.txt', 'a') as file_object:
